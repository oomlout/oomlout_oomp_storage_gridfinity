...
include <../../github/kennetek/gridfinity-rebuilt-openscad/src/core/gridfinity-baseplate.scad>
    ...
    difference() {
        union() {
            ...
            difference() {
                ...
                //haf row
                grid_size_2 = [3,1];
                pattern_grid(grid_size_2, [length, length], true, true) {
                    //echo("here");
                    //echo(grid_size);
                    if (minimal) {
                        //added a shift by aaron
                        translate([0, 42*1+42/2, 0]){
                            translate([0, 0, -TOLLERANCE/2]){
                                baseplate_cutter_half_height([length, length], baseplate_height_mm+TOLLERANCE);
                            }
                        }
                    }
                }
